...
char setupSsid[32] = "", setupPassword[17] = "", nonce[33] = "", setupToken[21] = "";
...
bool setupIdentity()
{
  ...
  snprintf(setupSsid, sizeof(setupSsid), "SmartPill_Setup_%02X%02X%02X", digest[0], digest[1], digest[2]);
...
void startSetup()
{
  ...
  setupActive = true;
  Serial.printf("[Setup] Connect to %s then open http://192.168.4.1\n", setupSsid);
}
