...
```text
Website (manual dispense; automatic schedule not implemented)
        |
        v
ESP32
...
| Buzzer | 25 | ตั้งเป็น OUTPUT/LOW; ยังไม่มี logic แจ้งเตือน |
| Confirm button | 32 | INPUT_PULLUP, ปุ่มต่อ GND; ยังไม่มี handler |
| Dispense button | 33 | INPUT_PULLUP, ปุ่มต่อ GND; ยังไม่มี handler |
| Cancel button | 27 | INPUT_PULLUP, กดลง GND เพื่อยกเลิก |
...
หน้าเว็บเรียก `dispenseMedicine()` โดยตรงและตอบ `202` เมื่อเริ่มรอบ, `400` เมื่อพารามิเตอร์ไม่ถูกต้อง,
`409` เมื่อกำลังทำงานหรือกด Cancel และ `503` เมื่อปิดการเคลื่อนไหวหรือ attach Servo ไม่สำเร็จ
ไม่มีคิวคำสั่งค้างให้ทำงานภายหลัง การตอบว่าเริ่มรอบไม่ใช่การยืนยันว่าเม็ดยาตกครบ
...
## IR และส่วนที่ยังไม่พัฒนา

มี IR 3 ตัวสำหรับตรวจเม็ดยาที่ตกจากจานของตนเอง โดย ESP32 จะรับผิดชอบอ่านค่าแทน Nano
ยังต้องยืนยันรุ่น แรงดัน ชนิดเอาต์พุต ระดับสัญญาณเมื่อตรวจพบ และระยะติดตั้งก่อนเพิ่ม firmware
ปัจจุบันจำนวนที่สั่งเป็นจำนวนรอบขยับ Servo ยังไม่มีการนับเม็ดยาจริงหรือหยุดเมื่อยาไม่ตก

ตารางยาอัตโนมัติ การตั้งเวลา RTC และ handler ของปุ่ม Confirm/Dispense ยังไม่ถูกพัฒนา
LCD จอที่สองแสดง `No schedule` จนกว่าจะมีโค้ดเรียก `showNextMedicine()`
การเริ่มระบบยังรอเชื่อมต่อ Wi-Fi ตามโค้ดเดิม ก่อนเข้าสู่ loop หลัก
